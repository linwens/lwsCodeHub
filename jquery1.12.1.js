...
jQuery.extend( {
	...
	isWindow: function( obj ) {
		/* jshint eqeqeq: false */
		//+/通过判断参数内是否包含window熟悉来判断参数是不是window对象，因为只有Window对象下才包含window属性。
	...
	type: function( obj ) {
		if ( obj == null ) {//+/针对IE6，如果是null就变成字符串
			return obj + "";
		}
		//+/三元判断，如果是引用类型就通过Object.prototype.toString去取class2type里的值，如果是基础类型就直接typeof
...
//+/填充class2type对象，用于存储数据类型值[object class]: class；配合Object.prototype.toString
//+/split(separator,howmany)方法用于把一个字符串分割成字符串数组。第一个参数是指定切割边界，第二个参数指定返回数组的最大长度。
...
function isArrayLike( obj ) {
	...
	//+/!!obj避免了传入空值报错，传入数据存在length属性，取length值
	var length = !!obj && "length" in obj && obj.length,
		type = jQuery.type( obj );
	//+/是函数或是window对象就返回false
	if ( type === "function" || jQuery.isWindow( obj ) ) {
		return false;
	}
	//+/length存在但是为0 也就是一个空数组 那么下标访问就没太大意义了 后面循环的时候肯定是进不去的
	//+/in操作符操作对象，那前面写属性值；操作数组前面写索引值，所以( length - 1 ) in obj可以判断是不是数组，是数组就返回true
